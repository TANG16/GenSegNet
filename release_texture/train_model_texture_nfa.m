function net = train_model_texture_nfa(config, net, imdb, getBatch)
...
%                                                        Train and validate
% -------------------------------------------------------------------------
z = randn(config.vis_dim_z,config.vis_dim_z, 1, config.nTileRow*config.nTileCol, 'single');
...
for epoch=1:opts.numEpochs
    ...
    train = opts.train;
    if numGpus <= 1
        [net, syn_mats] = process_epoch_texture_nfa(opts, getBatch, epoch, train, learningRate, imdb, net, syn_mats, config);
    ...
    if mod(epoch - 1, 20) == 0 || epoch == opts.numEpochs
        ...
        syn_mat = sampler(opts, config, net, epoch, z);        
